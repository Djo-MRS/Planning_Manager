defmodule Timemanager.WorkingtimesTest do
  ...
  describe "workingtimes" do
    ...
    test "create_workingtime/1 with valid data creates a workingtime" do
      valid_attrs = %{start: ~D[2024-10-07], end: ~D[2024-10-07]}

      assert {:ok, %Workingtime{} = workingtime} = Workingtimes.create_workingtime(valid_attrs)
      assert workingtime.start == ~D[2024-10-07]
      assert workingtime.end == ~D[2024-10-07]
    end
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      update_attrs = %{start: ~D[2024-10-08], end: ~D[2024-10-08]}

      assert {:ok, %Workingtime{} = workingtime} = Workingtimes.update_workingtime(workingtime, update_attrs)
      assert workingtime.start == ~D[2024-10-08]
      assert workingtime.end == ~D[2024-10-08]
    end
